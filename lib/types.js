...
function Types(opt_options, opt_types) {
  ...
  if (typeof opt_types === 'string') {
    opt_types = trees[opt_types];
  }
  opt_types = opt_types || trees.espree;

  this._compile(opt_types);
}
...
Types.prototype._compile = function(types) {
  ...
  for (i = 0; i < keysG.length; ++i) {
    ...
    if (property) {
      ...
      for (j = 0; j < this.group[type].length; ++j) {
        this.unique[this.group[type][j]].__group.push(property);
      }
